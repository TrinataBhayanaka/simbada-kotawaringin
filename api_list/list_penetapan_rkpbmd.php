...
if($_SESSION['ses_uaksesadmin']==1){
    ...
    $sWhere =" WHERE YEAR(a.tgl_usul) ='{$tahun}' AND status_usulan = 1 ";
    $condtn =" WHERE YEAR(tgl_usul) ='{$tahun}' AND status_usulan = 1 ";
}else{
    $kodeSatker = $_SESSION['ses_satkerkode'];
    $sWhere =" WHERE a.kodeSatker='$kodeSatker' AND YEAR(a.tgl_usul) ='{$tahun}' AND status_usulan = 1 ";
    $condtn =" WHERE kodeSatker='$kodeSatker' AND YEAR(tgl_usul) ='{$tahun}' AND status_usulan = 1 ";
}
...
while ($aRow = $DBVAR->fetch_array($rResult)) {
	...
	$detail="<a style=\"display:display\"  
		href=\"list_penetapan_aset.php?idus={$idus}&satker={$kodeSatker}\"	class=\"btn btn-info btn-small\" id=\"\" value=\"\" >
	  	...
	  	if($aRow['status_validasi'] == 0){
			...
			$row[] = "<center><span class=\"label $label\">$wrd </span></center>";
	      	...
	      	$unpenetapan="<a style=\"display:display\"  
				href=\"proses_usulan.php?idus={$idus}&satker={$kodeSatker}&flag=4\"	class=\"btn btn-danger btn-small proses\" id=\"\" value=\"\" >
			...
			$unverifikasi="<a style=\"display:display\"  
				href=\"proses_usulan.php?idus={$idus}&satker={$kodeSatker}&flag=3\"	class=\"btn btn-danger btn-small proses\" id=\"\" value=\"\" >
					<i class=\"fa fa-spinner\" align=\"center\"></i>&nbsp;&nbsp;Batal Verifikasi</a>";
			$Penetapan ="<a style=\"display:display\"  
				href=\"proses_usulan.php?idus={$idus}&satker={$kodeSatker}&flag=5\"	class=\"btn btn-success btn-small proses\" id=\"\" value=\"\" >
