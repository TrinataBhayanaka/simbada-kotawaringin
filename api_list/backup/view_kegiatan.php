...
$aColumns = array('idk','idp','tahun','kodeSatker','kd_kegiatan','kegiatan');
//$test = count($aColumns);
...
if ($satker != '' AND $tahun != '' AND $idp != ''){
	$sWhere=" WHERE tahun='$tahun' AND kodeSatker='$satker' AND idp='$idp'";
}else{
	$sWhere="";
...
if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {
     //$sWhere = "WHERE (";
	if ($satker != '' AND $tahun != ''){
		$sWhere=" WHERE tahun='$tahun' AND kodeSatker='$satker' AND idp='$idp' AND (";
	}else{
		$sWhere.="(";
	}
...
$sQuery = "
		...
		FROM   $sTable WHERE tahun='$tahun' AND kodeSatker='$satker' AND idp='$idp'";
	// 	echo $sQuery;
...
while ($aRow = $DBVAR->fetch_array($rResult)) {
	...
	$delete="<a style=\"display:display\" 
			href=\"delete_kegiatan.php?idk={$idk}&idp={$idp}&tahun={$tahun}&satker={$kodeSatker}\" onclick=\"return confirm('Hapus Data?');\"
			class=\"btn btn-danger btn-circle\" id=\"\" value=\"\" title=\"Hapus\">
			
			<i class=\"fa fa-trash simbol\">&nbsp;Hapus</i></a>";
	
	$edit="<a style=\"display:display\"  
			href=\"edit_kegiatan.php?idk={$idk}&idp={$idp}&tahun={$tahun}&satker={$kodeSatker}\"	class=\"btn btn-warning btn-small\" id=\"\" value=\"\" >
