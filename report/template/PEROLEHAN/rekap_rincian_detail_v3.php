	...
	<tr>
		   ...
		   <td style=\" text-align: center; font-weight: bold; width: \">11</td>
                   ...
                   <td style=\" text-align: center; font-weight: bold; width: \">19</td>
                    ...
                    <td style=\" text-align: center; font-weight: bold; width: \">24</td>
...
//
    ...
    foreach ($data_gabungan as $key => $value) {
        ...
        $data_level5[ $key_baru ][ 'total_pengadaan' ] += $data_gabungan[ $key ]['total_pengadaan'];
...
//Buat array gabungan --> level 4
    foreach ($data_level5 as $key => $value) {
        ...
        $data_level4[ $key_baru ][ 'nilai' ] += round($data_level5[ $key ][ 'nilai' ],2);
        $data_level4[ $key_baru ][ 'jml' ] += round($data_level5[ $key ][ 'jml' ],2);
        $data_level4[ $key_baru ][ 'ap' ] += round($data_level5[ $key ][ 'ap' ],2);
        $data_level4[ $key_baru ][ 'pp' ] += round($data_level5[ $key ][ 'pp' ],2);
        $data_level4[ $key_baru ][ 'nb' ] += round($data_level5[ $key ][ 'nb' ],2);
        ...
        foreach ($getdataRwyt as $valRwyt) {
            ...
            if($paramKd_Rwyt == 0 ||$paramKd_Rwyt == 30|| $paramKd_Rwyt == 2||$paramKd_Rwyt == 281 ||$paramKd_Rwyt == 291 || $paramKd_Rwyt == 7 || $paramKd_Rwyt == 21 || $paramKd_Rwyt == 29) {
               ...
               //  if( $action_riwayat=="Sukses Mutasi") {
                ...
                $nilaiPrlhnMutasiKurang = $valRwyt->NilaiPerolehan;
                $nilaiPrlhnMutasiKurangFix = ($nilaiPrlhnMutasiKurang);
                $transfer_keluar=$nilaiPrlhnMutasiKurang;
                //MUTASI PENYUSUTAN (Berkurang)
                $penyusutanBerkurang = $AkumulasiPenyusutan;
                $penyusutanBerkurangFix = ($penyusutanBerkurang);
...
//                $KOREKSI_TAMBAH=$valRwyt->NilaiPerolehan-$valRwyt->NilaiPerolehan_Awal;
            ...
            if(($paramKd_Rwyt == 50 || $paramKd_Rwyt == 51) && $status_masuk_penyusutan != 1) {
                ...
                $sejarah=$RIWAYAT[$paramKd_Rwyt];
                 if($TEXT_RIWAYAT!="")
                  $TEXT_RIWAYAT.=",$sejarah($paramKd_Rwyt) $tambahan_keterangan_riwayat $MUTASI_ASET_PENAMBAHAN  ";
                else
                  $TEXT_RIWAYAT.="$sejarah($paramKd_Rwyt) $tambahan_keterangan_riwayat $MUTASI_ASET_PENAMBAHAN ";
...
/*    $paramLog = "l.TglPerubahan >'$tglawalperolehan' and l.TglPerubahan <='$tglakhirperolehan'  and l.TglPembukuan>='$tglpembukuan'
    ...
    $paramLog = "l.TglPerubahan >'$tglawalperolehan' and l.TglPerubahan <='$tglakhirperolehan'  
				 AND l.Kd_Riwayat in (0,1,2,3,7,21,26,27,28,50,51,29,30,281,291,36,55,20,35) and l.Kd_Riwayat != 77 
				 and l.Aset_ID = '{$AsetId}' 
