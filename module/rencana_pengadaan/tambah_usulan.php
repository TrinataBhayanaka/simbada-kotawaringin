...
?>
	...
	function hierachy(){
			...
			if (data){
				for(i=0;i<data.length;i++){
					template+='<option value="'+data[i].idk+'" required="">'+data[i].kd_kegiatan+" "+data[i].kegiatan+'</option>';
				}
	...
	function secondhierachy(){
			...
			if (data){
				for(i=0;i<data.length;i++){
					template+='<option value="'+data[i].idot+'" required="">'+data[i].kd_output+" "+data[i].output+'</option>';
				}
			...
			<ul>
				...
				<li>
        			...
        			while($get_program = mysql_fetch_assoc($program)){
        				echo "<option value='$get_program[idp]' required=''>$get_program[kd_program] $get_program[program]</option>";
        			}
        			?>	
        			</select>
				</li>
