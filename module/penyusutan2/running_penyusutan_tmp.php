  ...
  if($status_running==0){
          switch ($kelompok) {
               case "Peralatan dan Mesin (B)":
                    ...
                    $status=exec("php running_penyusutan_server_custome_tmp.php B $Tahun $kodeSatker $id >> $path/log/berjalan-$TAHUN_AKTIF-penyusutan-B-$kodeSatker.txt &");
					header('Location: penyusutan.php');
					break;
			case "Gedung dan Bangunan (C)":
                   $query="update  penyusutan_tahun_berjalan  set StatusRunning=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php running_penyusutan_server_custome_tmp.php C $Tahun $kodeSatker $id >> $path/log/berjalan-$TAHUN_AKTIF-penyusutan-C-$kodeSatker.txt &");
                   header('Location: penyusutan.php');
				   break;
              ...
              case "Jalan, Irigrasi, dan Jaringan (D)":
                   $query="update  penyusutan_tahun_berjalan  set StatusRunning=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php running_penyusutan_server_custome_tmp.php D $Tahun $kodeSatker $id >> $path/log/berjalan-$TAHUN_AKTIF-penyusutan-D-$kodeSatker.txt &");
                   header('Location: penyusutan.php');
				   break;
			}
         // echo $status;
