...
function hapus_kontrak_kapitalisasi( $kontrak_id ) {
	...
	foreach ( $data_aset as $key =>$row ) {
		...
		list ( $NilaiPerolehan, $log_id )= get_log( $aset_id, $tglKontrak, $tabel ) ;

		$sql_aset="update aset set statusvalidasi=1, status_validasi_barang=1,
				 NilaiPerolehan='$NilaiPerolehan'  where aset_id='$aset_id'";
		$result=mysql_query( $sql_aset ) or die( mysql_error() );
...
function get_log( $aset_id, $tglPerubahan, $table ) {
	$sql="select log_id, TglPerubahan,NilaiPerolehan_Awal from log_$table
			 where Aset_ID='$aset_id' and TglPerubahan='$tglPerubahan' and kd_riwayat in(2,35,36)";
	$result=mysql_query( $sql ) or die( mysql_error() );
	while ( $data = mysql_fetch_array( $result, MYSQL_ASSOC ) ) {
		$NilaiPerolehan=$data['NilaiPerolehan_Awal'];
		$log_id=$data['log_id'];
	}
	return array ( $NilaiPerolehan, $log_id );
}
