  ...
  if($status_running==2){
          switch ($kelompok) {
            case "Peralatan dan Mesin (B)":
					...
					// exit;
                    $status=exec("php batal_penyusutan_tahun_pertama.php B $Tahun $kodeSatker $id >> $path/log/batal-penyusutan-B-$kodeSatker.txt &");
					header('Location: index_penyusutan_pertama.php');
					break;
			case "Gedung dan Bangunan (C)":
                   $query="update  penyusutan_tahun  set StatusRunning=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php batal_penyusutan_tahun_pertama.php C $Tahun $kodeSatker $id >> $path/log/batal-penyusutan-C-$kodeSatker.txt &");
                   header('Location: index_penyusutan_pertama.php');
				   break;
			case "Jalan, Irigrasi, dan Jaringan (D)":
                   $query="update  penyusutan_tahun  set StatusRunning=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php batal_penyusutan_tahun_pertama.php D $Tahun $kodeSatker $id >> $path/log/batal-penyusutan-D-$kodeSatker.txt &");
                   header('Location: index_penyusutan_pertama.php');
				   break;
			}
         // echo $status;
