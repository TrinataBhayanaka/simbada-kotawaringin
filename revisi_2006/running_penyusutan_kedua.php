  ...
  if($status_running==0){
          switch ($kelompok) {
               case "Peralatan dan Mesin (B)":
                    ...
                    $status=exec("php penyusutan_kedua.php B $Tahun $kodeSatker $id >> $path/log/$Tahun-berjalan-revisi-penyusutan-B-$kodeSatker.txt &");
					header('Location: index_penyusutan_kedua.php');
					break;
			case "Gedung dan Bangunan (C)":
                   $query="update  penyusutan_tahun_berjalan  set StatusRunning=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php penyusutan_kedua.php C $Tahun $kodeSatker $id >> $path/log/$Tahun-berjalan-revisi-penyusutan-C-$kodeSatker.txt &");
                   header('Location: index_penyusutan_kedua.php');
				   break;
              ...
              case "Jalan, Irigrasi, dan Jaringan (D)":
                   $query="update  penyusutan_tahun_berjalan  set StatusRunning=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php penyusutan_kedua.php D $Tahun $kodeSatker $id >> $path/log/$Tahun-berjalan-revisi-penyusutan-D-$kodeSatker.txt &");
                   header('Location: index_penyusutan_kedua.php');
				   break;
			}
         // echo $status;
